...
public:
    ...
    void accountHelper(UNode* node)const;
    void clearHelper(UNode* node);
    UNode* retrieveUserName(UNode* node,string username);
    UNode* insertNode(UNode* node, string username);
    void update(UNode* node);
    void removeUNode(UNode* node);
    UNode* leftRot(UNode*& node);
    UNode* rightRot(UNode*& node);
    int balanceFactor(UNode*& node);
